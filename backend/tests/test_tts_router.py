...
class TestSynthesize:
    ...
    def test_synthesize_with_preset_voice(self, mock_get_client, client):
        """When voice has no local file, it's sent as voice_id."""
        mock_resp = MagicMock()
        mock_resp.content = b"fake-wav-data"
        mock_resp.raise_for_status = MagicMock()
        mock_get_client.return_value = MagicMock(post=AsyncMock(return_value=mock_resp))
        ...
        call_kwargs = mock_get_client.return_value.post.call_args
        assert call_kwargs[1]["data"]["text"] == "hello"
        assert call_kwargs[1]["data"]["voice_id"] == "Binh"
        assert call_kwargs[1]["data"]["model"] == "vieneu:turbo"

    @patch("kurisuassistant.routers.tts.get_client")
    def test_synthesize_with_ref_audio(self, mock_get_client, client, tmp_path):
    ...
    @patch("kurisuassistant.routers.tts.get_client")
    def test_synthesize_text_only(self, mock_get_client, client):
    ...
    @patch("kurisuassistant.routers.tts.get_client")
    def test_synthesize_upstream_error_returns_502(self, mock_get_client, client):
        """Connection error to universal-voice returns 502."""
        mock_get_client.return_value = MagicMock(post=AsyncMock(side_effect=httpx.ConnectError("refused")))
...
class TestListVoices:
    @patch("kurisuassistant.routers.tts.get_client")
    def test_list_voices(self, mock_get_client, client):
        mock_resp = MagicMock()
        mock_resp.json.return_value = [
            {"id": "Binh", "name": "Binh (nam)", "model": "vieneu:turbo"},
        ]
        mock_resp.raise_for_status = MagicMock()
        mock_get_client.return_value = MagicMock(get=AsyncMock(return_value=mock_resp))
    ...
    @patch("kurisuassistant.routers.tts.get_client")
    def test_list_voices_with_model_filter(self, mock_get_client, client):
        mock_resp = MagicMock()
        mock_resp.json.return_value = []
        mock_resp.raise_for_status = MagicMock()
        mock_get_client.return_value = MagicMock(get=AsyncMock(return_value=mock_resp))
        ...
        call_kwargs = mock_get_client.return_value.get.call_args
        assert call_kwargs[1]["params"]["model"] == "vieneu:turbo"

    @patch("kurisuassistant.routers.tts.get_client")
    def test_list_voices_upstream_error(self, mock_get_client, client):
...
class TestListModels:
    @patch("kurisuassistant.routers.tts.get_client")
    def test_list_backends_filters_tts(self, mock_get_client, client):
        mock_resp = MagicMock()
        mock_resp.json.return_value = {
            "object": "list",
            "data": [
...
class TestHealthCheck:
    ...
    @patch("kurisuassistant.routers.tts.get_client")
    def test_health_error_returns_ok_false(self, mock_get_client, client):
        mock_get_client.return_value = MagicMock(get=AsyncMock(side_effect=httpx.ConnectError("refused")))
