...
**The suite never touches this checkout's `data/`.** The server resolves `data/` from the package, so a run from a checkout with real data used to write uploads among the real images and delete a real persona's character files through `DELETE /personas/{id}` (#307). `conftest.py` points `core.paths.DATA_DIR` at a temporary directory before anything else from the package is imported, and removes it when the session ends; `tests/test_suite_isolation.py` fails if any store (images, character assets, the drive, voices) still points into the checkout. A new store must take its path from `DATA_DIR` at import, never from the working directory.

Three markers, registered in `pytest.ini`:

- **unmarked** — pure unit tests, no services.
- **`db`** — needs Postgres: the migration tests and the *system tests* (`tests/test_system_chat.py`), which run the real app on a fresh database created for the session, create and activate their own account, log in as it, open `/ws/chat` and drive whole turns — streaming, thinking, the tool loop with its approval gate, compaction — with the model played by the mock Ollama. Beside it (#311): `test_system_auth.py` (refresh, the wire-protocol gate over HTTP and on the socket), `test_system_socket.py` (a tool the client runs, stopping a reply, the message queue and its cap, reconnecting mid-reply, images), `test_system_settings.py` (deleting conversations and messages, tool policies, the model routes, sub-agents, MCP servers, skills, persona export/import), `test_system_agents.py` (a sub-agent turn, skills, a real SSE MCP server's tool, memory consolidation end to end, automatic compaction) and `test_system_character.py` (the VRM store on the real database). New system tests take the `account` fixture — a fresh activated account per test, its Ollama at the mock — so nothing one test changes reaches the next; the session-wide `tester` account is for the older modules, which reset what they touch. CI provides Postgres (`backend-test.yml`); locally they skip unless `POSTGRES_HOST`/`POSTGRES_PORT` point at one. On CI (`CI` set) a missing Postgres fails the run instead: every such skip goes through `tests/postgres.py`'s `require_postgres`, and the workflow runs with `-rs` so any skip is listed with its reason (#307). A throwaway is `docker run --rm -d -p 127.0.0.1:55432:5432 -e POSTGRES_USER=kurisu -e POSTGRES_PASSWORD=kurisu -e POSTGRES_DB=kurisu pgvector/pgvector:pg16`.
