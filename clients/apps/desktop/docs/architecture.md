...
@kurisu/ui  (clients/packages/ui/src/components/)
  ...
  character/
    ...
    VrmSetupDialog.tsx     — "Set up {name} in 3D" (#242): five steps + Fine-tune over `vrm`, autosaved with `useDebouncedAutosave` (`vrm` minus the server-owned `model`/`clips`); uploads fold the returned config in at once. Presets and recipes are data in `@kurisu/models` `vrmSetup.ts`; the words in `vrmSetupText.ts`
    VrmPreview.tsx         — The editor's preview: its own `createVrmDriver` fed by the buttons, not the live feed; reloads only for a new model or clip list and `configure`s everything else
...
@kurisu/hooks  (clients/packages/hooks/src/)
  autosave.ts             — `createAutosaver` / `useDebouncedAutosave`: one save at a time, a change during a save re-arms once, `flush` on close
...
@kurisu/ui CharacterWindowApp — The second window's root: no login and no producers of its own (the access token is pushed to it over `character:session` and it fetches nothing until then); every IPC message is written into this renderer's copy of the character feed store, and one `CharacterSurface` per persona reads it; subtitle overlay from `SubtitleQueue`
@kurisu/ui character/     — The seam every character kind sits behind (#238)
  CharacterSurface.tsx    — Owns the box, a ResizeObserver and the frame loop: samples the feed (`sampleSpeech` on its own clock, thinking, the gesture burst not yet taken, faces) and calls `driver.update`; picks the driver by `character.kind`; reloads on `characterFingerprint` change; retries a failed load on the next session bump
