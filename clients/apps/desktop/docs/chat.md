...
- Each row has one conversation, managed via the `kurisu_persona_conversations` localStorage mapping (`Record<string, number>`, persona ID → conversation ID). The `'unbound'` key is the assistant's own: a conversation started while no persona was chosen stays there when the assistant answers it, and is re-keyed to a persona only when the first `stream_chunk` names one (the server's default answered).
...
- Backend auto-creates the conversation on first message with `conversationId=null`; a chosen persona goes along as `persona_id`, otherwise the server binds `assistant.default_persona_id` or lets the assistant answer. The first `StreamChunkEvent` saves the mapping. There is no new-chat persona picker — the chat header's persona sheet, whose first option is **Assistant** ("No persona — the assistant itself"), is a per-conversation override that `PATCH /conversations/{id}` persists; `persona_id: null` hands the conversation to the assistant, and the conversation leaves the bucket it was in.
