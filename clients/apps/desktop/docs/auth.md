...
- Before that, `App.tsx` calls `GET /version`; a wire-protocol mismatch shows `UpdateRequiredScreen` instead of the login form. When this app is the side behind, the screen offers the update itself (#264): "Update now" runs `useUpdateFlow` — `updater.checkForUpdates()` in the main process, the download's progress, then "Restart to update" (`quitAndInstall`) — and says "You already have the newest release (vX)" when nothing newer exists or a sentence when the check fails; an install that cannot replace itself (a `.deb`, an unpackaged run, the web build — `updater.canSelfUpdate()` answers, `null` updater on web) gets "Get the update", which opens the releases page and says so. When the server is the side behind nothing is offered. Its "Change server" button calls the auth store's `logout()` (tokens dropped, remember-me off) and clears the gate, so the login form renders with the stored Server URL editable. A 426 on any later request, or a 4426 socket close, raises the same screen the same way (#150; see [endpoints.md](endpoints.md))
