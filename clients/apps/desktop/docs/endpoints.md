...
- `GET /version` — `{backend_version, wire_protocol}`; the client's `WIRE_PROTOCOL` must match exactly (HTTP 426 / WebSocket close 4426 otherwise). `App.tsx` checks it at startup and, on a mismatch, replaces the UI with `UpdateRequiredScreen`, which says which side is behind (`@kurisu/api`'s `wireProtocol.ts`), offers the update when this app is behind (#264, [auth.md](auth.md)) and whose "Change server" signs out and returns to the login form. The same screen is raised after startup by a 426 on any request — the axios interceptor in `@kurisu/api`'s `client.ts` reads the server's numbers from the 426 body (`{detail: "wire_protocol_mismatch", client_wire_protocol, server_wire_protocol, backend_version}`) and calls the `onProtocolMismatch` callback — and by a 4426 socket close, which carries no numbers, so `wsManager` asks `apiClient.reportProtocolMismatch()` to fetch `/version` (exempt from the gate) first (#150). Settings → Account asks it once more, on its own, for the version rows (#257)
...
- `GET /personas`, `POST /personas`, `GET|PATCH|DELETE /personas/{id}`, `PATCH /personas/{id}/enabled`, `GET /personas/{id}/export` (`?character=true` for the v4 zip bundle, #248), `GET /personas/{id}/export/size`, `POST /personas/import` (a `.json`, multipart), `POST /personas/import/bundle` (a `.zip`, the raw body, no timeout) — Persona CRUD (presentation only). Settings → Personas' export dialog asks the size first and offers the character as a ticked box; Import sends a `.zip` to the bundle route and anything else to the JSON one
