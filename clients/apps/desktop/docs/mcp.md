...
**Starting one asks first.** A stdio server is a program running as the user, so `electron/mcp.ts` shows the exact command line (Cancel / Allow once / Always allow) unless it is already approved. Consent is keyed on the command, not the server's name. Playwright is pinned in `@kurisu/models` and its auto-start is off by default; it used to run `npx @playwright/mcp`, unpinned, on every connect.
