...
Workflows live at the repo root. `.github/workflows/release.yml` triggers on a tag `vX.Y.Z` (#256) — one release for both clients — checks that the tag's number is the backend's `__version__`, sets `package.json` version from the tag, builds the NSIS installer on `windows-latest` and AppImage + deb on `ubuntu-latest` (`electron-builder --publish never`), and publishes them with `latest.yml` / `latest-linux.yml` under the release `vX.Y.Z` on this repository beside the Android APK, uploading with the workflow's own token — no secret is involved. The committed `package.json` version is what a local build gets. `build.publish` in `package.json` names this repository: installed apps' `electron-updater` reads its `releases/latest` — the newest plain release, which the unified release always is (the Android transition alias `android-vX.Y.Z` is a pre-release and never takes that slot) — and fetches the files under that release's tag. `.github/workflows/desktop-test.yml` runs typecheck, vitest and the Playwright suite on pushes/PRs that touch `clients/apps/desktop/` or `clients/packages/`. It installs, typechecks and runs vitest from `clients/` — the lockfile is there, and the root scripts fan out to every workspace member, which is how the shared packages' suites (the boundary test, the protocol drift test) get run at all (#225). Auto-update: `electron-updater` checks GitHub Releases on app startup, downloads updates in the background, and prompts the user to restart via `UpdateDialog`.

## The parts that bite

Nine things that are not obvious from the code and have each cost a debugging session:
...
- **One WebSocket error code is not an error.** `NO_MODEL_SELECTED` means the account has never had a model chosen — every new account, since provisioning cannot pick one — so it renders as `NoModelPrompt` above the composer, whose button opens the model menu on the chat header in place, and the refused message goes back into the composer, instead of the red toast the other codes get ([docs/chat.md](docs/chat.md), #149). **Android mirrors this** with a bottom sheet off its own header (#197); neither client navigates away from the message, and neither Assistant page has a model picker.
- **Both clients say which release they are.** Settings → Account on the desktop and About on Android show the app's version, the backend's (`GET /version`) and the protocol number, and one sentence when app and backend are different releases (#257). The comparison is the release core only — `0.7.0-dev-abc` is the 0.7.0 release — in `@kurisu/models`' `versionParity.ts` and Android's `domain/version/VersionParity.kt`, each with the same tests. The app's number comes off the bridge (`appVersion`: package.json's version baked in at build time by `vite.config.ts`'s `__APP_VERSION__` define — the tag's number in a release build, the committed `0.0.1` in a local one, as with every other use of that field since #256; `app.getVersion()` would read Electron's own version in an unpackaged run; `null` in a browser, where the row is not shown). The mock backend reports the same committed number, so a local build and its mock agree.
...
- **Speech asks the server for nothing it did not choose, and says so when it fails.** A TTS request carries a `provider` only once the user picks one in Settings ("Default (server)" is the empty choice); the server's default model answers otherwise — the client used to fill in `vixtts`, Android `gpt-sovits`, and a route that never existed (#200). Synthesis and transcription failures are reported through `useTTS`'s `onError` and `micStore.error` into the chat's error toast, as one sentence from `describeSpeechFailure` (`@kurisu/api`). Voice input needs onnxruntime-web's `.mjs` files served as JavaScript: the packaged app's `file:` handler and the Vite dev server both do, but a browser build (`npm run build:web`) behind nginx does not unless `mime.types` knows `mjs` — without it Silero VAD never loads and the mic silently does nothing.
