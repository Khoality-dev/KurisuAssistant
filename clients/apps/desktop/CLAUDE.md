...
Workflows live at the repo root. `.github/workflows/desktop-build.yml` triggers on tags `desktop-vX.Y.Z`, sets `package.json` version from the tag, builds the NSIS installer on `windows-latest` and AppImage + deb on `ubuntu-latest`, and publishes them with `latest.yml` / `latest-linux.yml` as the release `desktop-vX.Y.Z` on this repository (#229): `electron-builder --publish never` builds, `softprops/action-gh-release` uploads with the workflow's own token, so no secret is involved. `build.publish` in `package.json` names this repository: installed apps' `electron-updater` reads its `releases/latest` and fetches the files under that release's tag (the tag need not be semver — the version is in `latest.yml`). That slot is the desktop's because the Android workflow marks its releases pre-releases, which `releases/latest` skips. `.github/workflows/desktop-test.yml` runs typecheck, vitest and the Playwright suite on pushes/PRs that touch `clients/apps/desktop/` or `clients/packages/`. It installs, typechecks and runs vitest from `clients/` — the lockfile is there, and the root scripts fan out to every workspace member, which is how the shared packages' suites (the boundary test, the protocol drift test) get run at all (#225). Auto-update: `electron-updater` checks GitHub Releases on app startup, downloads updates in the background, and prompts the user to restart via `UpdateDialog`.
