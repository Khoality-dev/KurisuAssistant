...
import { wsManager, StreamChunkEvent, DoneEvent, ErrorEvent, ConnectedEvent, ToolApprovalRequestEvent, ContextInfoEvent } from '@kurisu/api';
...
export function useStreamingChat({
  ...
  const handleSend = useCallback(async (text: string, imageFiles: File[]) => {
    ...
    if (isStreamingRef.current) {
      ...
      setQueuedMessages(prev => [...prev, { role: 'user', content: trimmed, images: [], queued: true, _clientKey: newId() }]);
