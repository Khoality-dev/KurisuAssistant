...
export function initAppToolsHandler(): void {
  ...
  if (initialized || !bridge.appTools) return;
  initialized = true;

  bridge.appTools.onExecute(async (data: AppToolCall) => {
    ...
    bridge.appTools!.sendResult(data.callId, result);
  });
  ...
  bridge.hostTools?.onDiffReview?.((data) => {
