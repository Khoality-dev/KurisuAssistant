...
- Build types × flavors:
  ...
  - Shippable build: `./gradlew assembleProdRelease` (needs `.env` with the release keystore). **Without the keystore the build fails**, naming the values it could not find, instead of substituting the debug key that ships in every SDK install — that produced an installable APK nothing could vouch for, and which no properly signed release can ever upgrade (#100). Debug builds are untouched: `assembleDevDebug`, the unit tests and an IDE sync all work on a machine with no signing material at all.
...
- Prod release: one tag for everything (#256). Push the tag `vX.Y.Z` on `main` — no bump commit, the number lives only in the tag (#291); the root `.github/workflows/release.yml` runs `assembleProdRelease -Pkurisu.releaseVersion=X.Y.Z` — `versionName` becomes X.Y.Z and `versionCode` is derived as X·10000 + Y·100 + Z (rising with every release while Y and Z stay below 100; the committed `versionCode`/`versionName` in `app/build.gradle.kts` are the local and dev-flavor fallback) — and publishes the APK under the release `vX.Y.Z` beside the desktop installers. `UpdateRepository` lists the repository's releases and `AndroidReleases` takes the newest `vX.Y.Z` carrying an APK (a release without one, like the 2025 `v0.0.x` ones, is not ours). Apps from before #256 only recognise `android-v*`, so the workflow also publishes the APK as a pre-release `android-vX.Y.Z` until the repository variable `ANDROID_LEGACY_ALIAS` is `false`; remove that job and the `android-v` branch of `AndroidReleases` together once no installed app predates the first unified release. Signing needs two secrets, `KURISU_KEYSTORE_BASE64` and `KURISU_KEYSTORE_PASSWORD`: the key alias is read from the keystore at build time and the key password is the store password (PKCS12 has one). If the keystore secrets are absent the tagged run now **fails at the build step** rather than publishing a debug-signed APK to that channel (#100).

## Testing

- **Unit tests** live in `app/src/test/`. Use Robolectric (`@RunWith(AndroidJUnit4::class)`) only when you need a Context; pure logic should stay plain JVM. Existing coverage: `SentenceSplitter`, `NarrationStripper`, `WavParser`, `AmplitudeCurveComputer`, `AnimationMigration`, `CharacterConfigKind` (the one reader of `character_config`, #235), `ChatStreamProcessor` (including the `NO_MODEL_SELECTED` code reaching `StreamingState`), `WebSocketManager`, `VoiceInteractionManager`, `VoiceCountdown`, `ConversationsViewModel`, `AssistantViewModel`, `PersonasViewModel`, `ChatViewModel` (persona override, and the assistant answering with no persona — #302; attached images sent as base64 and shown in the bubble — #308), `ToolRailModel`, `SlashCommands`, `PatchBody`, `UpdateRequiredCopy`, `UpdateFlow` (the gate's offer, #264), `WireProtocolInterceptor` (426 body → `ProtocolMismatchSignal`), `ReleaseSigningRuleTest` (reads `build.gradle.kts` and fails if the debug signing config is ever assigned again, or the release guard removed — #100)
...
- **No model chosen yet** — a new account's first message is refused by the server with `NO_MODEL_SELECTED`, and both clients answer it with a prompt onto the model picker rather than an error (#149). The picker is on the chat header on both clients — a bottom sheet here, a menu on desktop — and the prompt opens it in place: the chat banner turns `secondaryContainer` and "Choose a model" opens the sheet; on desktop the bar above the composer opens the menu. Neither navigates away, and the Assistant page carries no model picker on either client (#197)
- **About says which release this is, and when the backend's differs** — `ui/about/AboutScreen` shows the app's `BuildConfig.VERSION_NAME`, the backend's version and wire number from `GET /version`, and one sentence from `domain/version/VersionParity` when app and backend are different releases (release core only, so a dev stamp does not warn against its own backend). The desktop's Settings → Account carries the same rows (#257)
- **A failure that is not the API's is still a sentence** — `apiErrorMessage` (`ui/assistant/ApiError.kt`) reads the API's `detail` first and, with none, `domain/net/RequestFailure` names a proxy's 403, a dead address, an untrusted certificate or a timeout instead of "HTTP 403 Forbidden" (#263). The desktop's `describeRequestFailure` in `@kurisu/api` says the same sentences (`docs/patterns.md`)
- **Update gate is not a dead end** — both clients answer a wire-protocol mismatch with a screen that names which side to update, offers the update itself when this app is the side behind (check → download with progress → install/restart, "already the newest", a failure sentence; #264) and offers "Change server": sign out, back to the login form with the server URL editable (#150). Here it is `ui/version/UpdateRequiredScreen` rendered by `MainActivity` in place of the `NavHost`, and a live HTTP 426 or WebSocket 4426 raises it too via `ProtocolMismatchSignal`; on desktop the same control sits on `UpdateRequiredScreen.tsx` and the axios interceptor handles 426
- **Kurisu Drive is missing here, and it is half of #17.** The server has account-scoped file storage and the desktop browses it as a second root beside "This computer" (`backend/docs/drive.md`, `clients/desktop/src/api/fileSource.ts`). Android has no file surface at all beyond the two `GetContent()` image pickers in `ChatInput.kt` and `PersonasScreen.kt` — which makes it the client where a server-side drive is the *only* way to get a file in. It owes browsing, upload and download against the same `/drive/*` endpoints; the mock backend already serves them (`clients/desktop/tests/mock/server.ts`), so the instrumented suite can drive it. Its own issue; #17 closes when it lands
...
- **ASR Mode** (`kurisu_asr_mode`: "fixed" | "routing"): Fixed shows a single model dropdown (`kurisu_asr_fixed_model`); Routing shows a per-language mapping table (`kurisu_asr_model_map`, JSON-encoded `List<AsrLanguageModelEntry>`). Models populated from `GET /asr/models`, and — since #200 — actually sent as `?model=` by `CoreService.chooseAsrModel` (`domain/audio/AsrModelSelection`, unit-tested), with `POST /asr/detect-language` as the first leg of routing, as on desktop
- **TTS model** (`kurisu_tts_backend`): blank means the server's default and the request names no `provider`; the list is `GET /tts/models` on both clients. Speech failures — synthesis or transcription — are one sentence in the chat error banner (`CoreState.speechErrors`), as the desktop's toast (#200). The viXTTS "Emotion" controls and their keys (`kurisu_tts_emo_*`) are gone from both clients (#243): the backend's `POST /tts` never declared those fields at any revision (`backend/docs/tts.md`), and here the queue never even sent them
