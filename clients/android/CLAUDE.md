...
- **TTS model** (`kurisu_tts_backend`): blank means the server's default and the request names no `provider`; the list is `GET /tts/models` on both clients. Speech failures — synthesis or transcription — are one sentence in the chat error banner (`CoreState.speechErrors`), as the desktop's toast (#200). The viXTTS "Emotion" controls and their keys (`kurisu_tts_emo_*`) are gone from both clients (#243): the backend's `POST /tts` had dropped those fields since #212, and here the queue never even sent them
