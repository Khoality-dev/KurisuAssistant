...
- **TTS model** (`kurisu_tts_backend`): blank means the server's default and the request names no `provider`; the list is `GET /tts/models` on both clients. Speech failures — synthesis or transcription — are one sentence in the chat error banner (`CoreState.speechErrors`), as the desktop's toast (#200)
