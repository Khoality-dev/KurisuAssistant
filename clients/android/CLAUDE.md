...
- Prod release: one tag for everything (#256). Bump `__version__` in `backend/kurisuassistant/version.py`, merge, push the tag `vX.Y.Z`; the root `.github/workflows/release.yml` runs `assembleProdRelease -Pkurisu.releaseVersion=X.Y.Z` — `versionName` becomes X.Y.Z and `versionCode` is derived as X·10000 + Y·100 + Z (rising with every release while Y and Z stay below 100; the committed `versionCode`/`versionName` in `app/build.gradle.kts` are the local and dev-flavor fallback) — and publishes the APK under the release `vX.Y.Z` beside the desktop installers. `UpdateRepository` lists the repository's releases and `AndroidReleases` takes the newest `vX.Y.Z` carrying an APK (a release without one, like the 2025 `v0.0.x` ones, is not ours). Apps from before #256 only recognise `android-v*`, so the workflow also publishes the APK as a pre-release `android-vX.Y.Z` until the repository variable `ANDROID_LEGACY_ALIAS` is `false`; remove that job and the `android-v` branch of `AndroidReleases` together once no installed app predates the first unified release. Signing needs two secrets, `KURISU_KEYSTORE_BASE64` and `KURISU_KEYSTORE_PASSWORD`: the key alias is read from the keystore at build time and the key password is the store password (PKCS12 has one). If the keystore secrets are absent the tagged run now **fails at the build step** rather than publishing a debug-signed APK to that channel (#100).
...
>>>>>>> 8c2d91d (feat: the update-required screen offers the update instead of asking for it (#264))
