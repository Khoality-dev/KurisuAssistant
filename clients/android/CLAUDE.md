...
- Prod release: bump `versionCode`/`versionName`, commit, then push tag `android-v<versionName>`. The root `.github/workflows/android-release.yml` builds `assembleProdRelease` and publishes it as the release `android-v<versionName>` on this repository, marked a pre-release on purpose (#229): `releases/latest` must stay the desktop's, since its `electron-updater` can read nothing else, so `UpdateRepository` lists the repository's releases and takes the newest `android-v*` (`AndroidReleases`, with its own unit test). Signing needs two secrets, `KURISU_KEYSTORE_BASE64` and `KURISU_KEYSTORE_PASSWORD`: the key alias is read from the keystore at build time and the key password is the store password (PKCS12 has one). If the keystore secrets are absent the tagged run now **fails at the build step** rather than publishing a debug-signed APK to that channel (#100).
