...
- **Attached images travel as base64, as on desktop.** The composer holds the picker's `content://` URIs (`ChatUiState.selectedImages`); `ChatViewModel.sendMessage` keeps them as strings, and `doSend` reads each off the main thread (`ioDispatcher`, a seam for tests) and sends the bytes base64-encoded in `chat_request.images` — the backend stores them and the saved message carries their uuids. The user's bubble shows the URIs themselves until the `done` reload replaces them with the uuids (`MessageBubble` loads anything with a scheme as-is, a bare uuid from `/images/{uuid}`). An image over the backend's 16 MB ceiling, or one that cannot be read, is refused with a banner and the draft goes back into the composer. A turn without images still starts synchronously. Until #308 the list sent was always empty: a picked image was silently dropped
...
- **What the login screen says when it fails** (#263): `ui/assistant/ApiError.kt`'s `apiErrorMessage` is the one place every ViewModel turns an exception into a sentence. The API's JSON `detail` wins; with none, `domain/net/RequestFailure` names the kind of failure before the exception's own wording is considered — a proxy's HTML 403 → "Something in front of the server refused this device (HTTP 403). Check the server address, and whether the operator's proxy allows your network."; a bare 401 → "The server refused the credentials."; 404 → "No KurisuAssistant server answers at this address (HTTP 404)."; 502/503/504 → "The server is not reachable behind its proxy (HTTP N)."; another 5xx → "The server failed (HTTP N)."; `ConnectException`/`UnknownHostException` → "Nothing answered at <the URL typed on the login form>. Check the address and that the server is running."; an `SSLHandshakeException`/`CertPathValidatorException` anywhere in the cause chain → "The server's certificate is not trusted by this app."; `SocketTimeoutException` → "The server did not answer in time." `LoginViewModel` passes the typed URL as `origin`; the other callers leave it out. Unit-tested in `RequestFailureTest` and `ApiErrorTest`; the desktop's `describeRequestFailure` says the same sentences
...
- **The persona's feeling arrives, and nothing reads it yet** (#243): for a VRM persona with the emotion channel on, the backend strips the tags the model wrote and forwards a cue — `emotion` / `emotionAt` on `StreamChunkEvent` (the chunk whose content starts there) and `Message.emotionCues` in the history (`data/model/ApiModels.kt` `EmotionCue`). `emotion` is one of the six VRM preset names; `at` is an offset into that LLM round's accumulated assistant text in UTF-16 code units — Kotlin's `String.length`, so `substring(at)` is exact. A tool call starts a new round and the offsets restart at 0. Both fields are optional and decode to null on an older backend (`EmotionCueTest`). `ChatStreamProcessor` ignores them today; turning a cue into an expression when that sentence is *spoken* is the VRM renderer's job in #246
