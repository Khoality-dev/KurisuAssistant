...
- The built-in MCP server (`electron/mcpServer.ts`) publishes `host_bash` and every app tool, so it is treated as a privileged endpoint: loopback bind, no CORS headers, browser-header rejection, bearer token. It used to bind every interface with `Access-Control-Allow-Origin: *` and no auth, which put those tools within reach of the LAN and of any page the user visited. Keep all four properties together — dropping any one of them reopens it.
