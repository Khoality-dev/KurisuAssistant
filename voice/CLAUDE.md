...
The speech service: **universal-voice**, the one container `--profile voice` starts. Recognition (faster-whisper) and every synthesis backend — viXTTS, GPT-SoVITS, VieNeu — run in this process (#203); the backend's `kurisuassistant/speech/` package drives it over the Compose network (port 14213, never published, no authentication of its own — the API authenticates and this trusts the network), and #212 makes each backend its own container behind that package. It was its own repository, universal-asr, until #202, with viXTTS and GPT-SoVITS as further containers behind it: Kurisu was its only consumer, a backend release tag could not pin any of the three, and every change to the speech contract needed a PR in each repository.
...
- [API](docs/api.md) — every route, its request and response, and which backend route calls it.
...
```
universal_voice/
  main.py              the app; the lifespan starts one preload thread (the default ASR model's pull, then the TTS_PRELOAD loads) and the residency sweeper, and serves meanwhile
  config.py            every setting, read from the environment once at import
  scheduler.py         residency: resident / offloaded / unloaded per model, an LRU cap on resident synthesis models, idle sweeping (#207); load / offload / unload on request, 409 while in use (#218)
  routers/
    transcription.py   /asr, /asr/detect-language (raw PCM), /v1/audio/* (OpenAI-shaped uploads)
    tts.py             /tts/synthesize, /tts/voices
    health.py          /health, /v1/models (+ pull, delete), /v1/models/{id}/load|offload|unload (#218)
  models/
...
- **One image, any subset of engines** — `UVOICE_ENGINES` (#218) names what an instance runs (`whisper`, `vixtts`, `gpt-sovits`, `vieneu`; all by default). What is not named is not registered, listed or preloaded, and recognition routes answer 404 without `whisper`. This is how #212 runs one container per engine from this image.
...
- **Nothing idle stays on the GPU.** Every use of a model goes through `scheduler.use(model)` (#207): at most `UVOICE_TTS_MAX_RESIDENT` synthesis models are on the device (default 1, LRU eviction), an idle model is parked in CPU memory after `UVOICE_OFFLOAD_AFTER_SECONDS` and dropped after `UVOICE_UNLOAD_AFTER_SECONDS`, and a model in use is never touched. The same moves are on the wire as `POST /v1/models/{id}/load|offload|unload` (#218), 409 while the model is in use, for the API that will hold the cap across engines (#212). A new backend implements `load()` (idempotent, and it must bring an offloaded model back), `offload() -> bool` and `unload()`, sets `can_offload`, and lists its voices without loading (or sets `voices_need_weights`); call the model only inside `scheduler.use`. `docs/models.md` "Residency".
- **A request's fault is a `ValueError`.** The router turns it into a 400 with the text, which the backend passes to the client as the reason; anything else is a 500, which the backend reports as an outage (#215, #218). An unknown model, an unsupported language, an unknown or missing preset, GPT-SoVITS with no reference clip, text that normalises to nothing — all `ValueError`.
- **The wire contract with the backend** is `POST /asr` (Int16 PCM, 16 kHz, mono, as `application/octet-stream`; `?language`, `?model`, `?initial_prompt`), `POST /asr/detect-language`, `GET /v1/models`, `POST /tts/synthesize` (multipart: `text`, `model`, `voice_id`, `language`, `ref_audio`, `ref_text`), `GET /tts/voices`, `GET /health`. `backend/tests/test_tts_router.py` and `test_asr_router.py` and this package's `tests/test_routers.py` pin the two ends; change both in the same PR. The backend sends text in chunks of about 200 characters, already cut at paragraph and sentence boundaries (#215), so `split_text` here is the identity on what it receives. The model ids — `vixtts`, `gpt-sovits`, `vieneu:<mode>` — are stored in both clients' settings; renaming one is a client migration.
