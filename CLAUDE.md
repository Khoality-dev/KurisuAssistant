...
- Rolling Context Compaction — When context reaches 90% of model's context window, auto-compacts conversation into ~10% summary via inline LLM call. `Conversation.compacted_context` (rolling summary) + `compacted_up_to_id` (message watermark). `_load_context_messages()` loads compacted context + messages after watermark. Compacted context injected into agent system prompt via `AgentContext.compacted_context`. `ContextInfoEvent` sends token usage to client for real-time display.
